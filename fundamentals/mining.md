...
Our algorithm, [Ethash](/concepts/ethash/ethash.md) (previously known as Dagger-Hashimoto), is based around the provision of a large, transient, randomly generated dataset which forms a DAG (the Dagger-part), and attempting to solve a particular constraint on it, partly determined through a block's header-hash.
...
These are formally documented on the [JSON-RPC API](/json-rpc/api.md) wiki article.
